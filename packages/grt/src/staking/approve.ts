...
export async function approve(payload: WithApi<WithBridge<ApproveDto>>): Promise<void> {
  ...
  const transactionParams = await apiStaking.approve({ address, asset: Assets.GRT });
  ...
  if (__DEV__) {
    ...
    console.log(`Approving GRT token: max fee = "${Number(maxFee)} ETH"`);
  }

  await submitTransaction({ bridge, address, transactionParams });

  if (__DEV__) {
    console.log(`Successfully approved GRT token`);
  }
}
