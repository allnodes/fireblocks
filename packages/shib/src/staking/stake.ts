...
export async function stake(payload: WithApi<WithBridge<StakeDto>>): Promise<void> {
  ...
  const transactionParams = await apiStaking.stake({ address, asset: Assets.SHIB, amount });
  ...
  if (__DEV__) {
    ...
    console.log(`Staking SHIB token: max fee = "${Number(maxFee)} ETH"`);
  }

  await submitTransaction({ bridge, address, transactionParams, });

  if (__DEV__) {
    console.log(`Successfully staked SHIB`);
  }
}
